...
const allVariants$1 = Object.keys(allSymbols).sort((a, b) => b.length - a.length);
...
function getModifiers(parsableDescriptor) {
	...
	const descriptorRegex = new RegExp(
		allVariants$1.map(escapeRegex).join('|'),
		'g'
	);
...
function getFifths(allModifiers$1, altIntervals) {
	...
	if (
		...
		shouldAlter(allModifiers$1, altIntervals.fifthFlat)
	) {
		fifths.push('b5');
	}
	if (
		hasOneOf(allModifiers$1, [allModifiers.aug, allModifiers.fifthSharp]) ||
		shouldAlter(allModifiers$1, altIntervals.fifthSharp)
	) {
		fifths.push('#5');
	}
...
function getNinths(allModifiers$1, altIntervals) {
	...
	if (
		...
		shouldAlter(allModifiers$1, altIntervals.ninthFlat)
	) {
		ninth.push('b9');
	}
	if (
		hasOneOf(allModifiers$1, [allModifiers.ninthSharp]) ||
		shouldAlter(allModifiers$1, altIntervals.ninthSharp)
	) {
		ninth.push('#9');
	}
	return ninth;
...
function getElevenths(allModifiers$1, altIntervals) {
	...
	if (
		...
		shouldAlter(allModifiers$1, altIntervals.eleventhSharp)
	) {
		elevenths.push('#11');
	}
	return elevenths;
...
function getThirteenths(allModifiers$1, altIntervals) {
	...
	if (
		...
		shouldAlter(allModifiers$1, altIntervals.thirteenthFlat)
	) {
...
function shortenNormalized(chord) {
	...
	if (isSus2(chord)) {
		...
		descriptor = chord.formatted.descriptor.replace(
			/7sus|9sus/,
...
const notes$1 = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];
