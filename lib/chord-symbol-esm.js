...
const allVariants$1 = Object.keys(allSymbols)
	.sort((a, b) => b.length - a.length);
...
function getModifiers(parsableDescriptor) {
	...
	const descriptorRegex = new RegExp(allVariants$1.map(escapeRegex).join('|'), 'g');
...
function getFifths(allModifiers$1, altIntervals) {
	...
	if (hasOneOf(allModifiers$1, [allModifiers.dim, allModifiers.halfDim, allModifiers.fifthFlat]) || shouldAlter(allModifiers$1, altIntervals.fifthFlat)) {
		fifths.push('b5');
	}
	if (hasOneOf(allModifiers$1, [allModifiers.aug, allModifiers.fifthSharp]) || shouldAlter(allModifiers$1, altIntervals.fifthSharp)) {
		fifths.push('#5');
	}
...
function getSixth(allModifiers$1) {
	const sixth = [];
	if (hasOneOf(allModifiers$1, [allModifiers.add6, allModifiers.add69]) && !isExtended(allModifiers$1)) {
		sixth.push('6');
	}
	return sixth;
}
...
function getNinths(allModifiers$1, altIntervals) {
	...
	if (hasOneOf(allModifiers$1, [allModifiers.ninthFlat]) || shouldAlter(allModifiers$1, altIntervals.ninthFlat)) {
		ninth.push('b9');
	}
	if (hasOneOf(allModifiers$1, [allModifiers.ninthSharp]) || shouldAlter(allModifiers$1, altIntervals.ninthSharp)) {
		ninth.push('#9');
	}
	return ninth;
}
...
function getElevenths(allModifiers$1, altIntervals) {
	const elevenths = [];
	if (hasOneOf(allModifiers$1, [allModifiers.thirteenth]) && !hasMajorIntent(allModifiers$1)) {
		elevenths.push('11');

	} else if (hasOneOf(allModifiers$1, [allModifiers.eleventh, allModifiers.add11])) {
		elevenths.push('11');
	}
	if (hasOneOf(allModifiers$1, [allModifiers.eleventhSharp]) || shouldAlter(allModifiers$1, altIntervals.eleventhSharp)) {
		elevenths.push('#11');
	}
	return elevenths;
}
...
function getThirteenths(allModifiers$1, altIntervals) {
	...
	if (
		...
		|| (hasOneOf(allModifiers$1, [allModifiers.add6, allModifiers.add69]) && isExtended(allModifiers$1))
	) {
		thirteenths.push('13');
	}
	if (hasOneOf(allModifiers$1, [allModifiers.thirteenthFlat]) || shouldAlter(allModifiers$1, altIntervals.thirteenthFlat)) {
		thirteenths.push('b13');
	}
	return thirteenths;
}

function shouldAlter(allModifiers$1, isDegreeAlterated) {
	return (allModifiers$1.includes(allModifiers.alt) && isDegreeAlterated);
}
...
function isExtended(allModifiers$1) {
	return hasOneOf(allModifiers$1, [allModifiers.seventh, allModifiers.ninth, allModifiers.eleventh, allModifiers.thirteenth]);
}
...
function normalizeDescriptor(chord) {
	...
	if (isPowerChord(chordIntervals)) {
		...
		const { adds, alterations } = getAddsAndAlterations(chordIntervals, baseIntervals, quality);
...
function getChordQuality(allIntervals, chord, isSuspended, omits) {
	...
	return find_1(intervalsToQualities, o => hasAll(intervalsForQualityDetection, o.qualityIntervals));
}
...
function isExtended11th(allIntervals) {
	return hasOneOf(allIntervals, '11')
		&& hasOneOf(allIntervals,['b9', '9', '#9']);
}
...
function nameIndividualChordNotes(chord) {
	...
	const refNotes = (rootNoteToScaleAccidentals[rootNote][minMaj] === 'sharp') ? notesSharp : notesFlat;
...
function simplify(level = 'none', chord) {
	...
	const intervals = difference_1(chord.normalized.intervals, intervalsToRemove[level]);
...
function transposeNote(note, value, useFlats) {
	const noteIndex = notes$1.indexOf(note);
	...
	const transposed = notes$1[correctedTransposedIndex];
