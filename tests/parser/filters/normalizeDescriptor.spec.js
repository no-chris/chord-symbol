...
describe('normalizeDescriptor', () => {
	...
	describe.each(
		/* prettier-ignore */ [
			...
			['Altered', 'Calt', { quality: 'dominant7', alterations: ['b5', '#5', 'b9', '#9', '#11', 'b13'] }],
			['Altered (omit3)', 'Calt(omit 3)', { quality: 'dominant7', alterations: ['b5', '#5', 'b9', '#9', '#11', 'b13'] , omits: ['3']}],
			...
			['Sus', 'C4', { quality: 'major', isSuspended: true }],
	...
	describe.each([
		...
		[['b5', '#5', 'b9', '#9', 'b13']],
		[['b5', '#5', 'b9', '#9', '#11', 'b13']],
	])('Atl intervals', (altIntervals) => {
		test('Calt should yield alterations ' + altIntervals.join(', '), () => {
			const parseChord = getParseChord(altIntervals);
			const chord = parseChord('Calt');
			const normalized = normalizeDescriptor(chord).normalized;

			expect(normalized.alterations).toEqual(altIntervals);
		});
	});
});
